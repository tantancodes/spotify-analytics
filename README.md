...
- **Frontend** (`frontend/`) — Next.js (App Router) + TypeScript, running as a separate app on port 3000, talking to this API over CORS. A plain HTML/JS version still exists at `static/index.html` served directly by FastAPI on `/`, kept as a dependency-free fallback, but `/api/callback` redirects to the Next.js app by default.
...
FRONTEND_URL=http://localhost:3000   # optional -- only needed if the Next.js app runs somewhere other than the default
```
...
cd frontend
npm install
cp .env.local.example .env.local   # only needs editing if your backend isn't on the default port
npm run dev
```
